...
\title{Pizzo: A Gamified Chinese Postman Route Planner with a Blackjack Mini-game}

\author{\IEEEauthorblockN{Tarik Agi\'c}
\IEEEauthorblockA{Project Description (Artifical Intelligence)\\
Student ID: 20108}
}
...
To minimize fuel cost and exposure risk, Toni seeks the shortest closed walk that covers every street at least once---the \emph{Chinese Postman Problem} (CPP)~\cite{ref_cpp_wiki,ref_mit_urban}.
...
\begin{IEEEkeywords}
Chinese Postman Problem, Euler circuit, Dijkstra, minimum perfect matching, Hierholzer, Blackjack, expectimax, expected value, GUI.
\end{IEEEkeywords}
...
The undirected CPP asks for the minimum-cost closed walk that traverses every edge at least once~\cite{ref_cpp_wiki,ref_mit_urban}.
...
A ``script''-like automatic drawing routine exists directly in the GUI code.
...
Figures~\ref{fig:main} and~\ref{fig:bj} show the application in both modes.
...
\bibitem{ref_cpp_wiki}
Wikipedia, ``Chinese postman problem.'' [Online]. Available: \url{https://en.wikipedia.org/wiki/Chinese_postman_problem}. Accessed: 2026-02-16.
